...
import { Typography, Tabs, List, Tag, Card, Spin, Space } from 'antd';
...
export default function SearchResults() {
  ...
  const [data, setData] = useState<any>({ posts: [], questions: [], resources: [], announcements: [] });
  ...
  const total = (data.posts?.length || 0) + (data.questions?.length || 0) + (data.resources?.length || 0) + (data.announcements?.length || 0);
